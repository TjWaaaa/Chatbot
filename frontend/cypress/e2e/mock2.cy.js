...
describe('Mock socket', () => {
	...
	socket.socketClient.on('username', (name) => {
		...
		socket.socketClient.emit('is_online', '🔵 <i>' + username + ' join the chat..</i>');
	});
	...
	it('chats', () => {
		cy.intercept('/scripts/app.js', (req) => {
			...
			req.continue((res) => {
				res.body = res.body.replace("io.connect('http://localhost:8080')", 'window.testSocket');
			});
		}).as('appjs');
		...
		cy.should(() => {
			...
			socket.socketClient.emit('chat_message', '<strong>Cy</strong>: Mock socket works!');
