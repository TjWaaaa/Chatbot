...
describe('User Endpoints', () => {
	...
	// const updateUserEmailById = jest.spyOn(user, 'updateUserEmailById');
	// const deleteUserById = jest.spyOn(user, 'deleteUserById');
	// const createOnboardingMessages = jest.spyOn(createOnboardingMessages);
	...
	// 	getUserById.mockResolvedValue(user1);
	// 	updateUserEmailById.mockResolvedValue();
	...
	// 	expect(response.text).toContain('User updated');
	// });

	// it('should delete user with email e2e@testc.de', async () => {
	// 	deleteUserById.mockResolvedValue(true);

	// 	const response = await request(app).patch('/users/1').set({ 'csrf-token': 'cbacb' }).expect(200);
